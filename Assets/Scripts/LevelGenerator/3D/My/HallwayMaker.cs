...
public class HallwayMaker : MonoBehaviour
{
    ...
    private void RoomWallDetected(RaycastHit hit, Transform checker)
    {
        if (makeRoomDoor)
        {
            ...
            GameObject door = Instantiate(doorPrefab, hit.collider.transform.position, Quaternion.LookRotation(hit.normal), hit.transform.parent.parent.Find("parentDoors"));
            Destroy(checker.parent.gameObject);

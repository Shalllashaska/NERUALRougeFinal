...
public class LevelGenarator3D : MonoBehaviour
{
    ...
    private void Awake()
    {
        Debug.Log(GameSettings.Seed);
        random = new Random(GameSettings.Seed);
        roomCount = random.Next(7, 16);
        int x = random.Next(size.x - 10, size.x + 10);
        int y = random.Next(size.y - 10, size.y + 10);
        int z = random.Next(size.z - 10, size.z + 10);
